...
class SnakeGame extends EventEmitter {
    ...
    handleKeyPress(event) {
        ...
        const gameKeys = ['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight', 'Space', 'KeyR', 'KeyM'];
    ...
    levelUp() {
        this.level++;
        this.foodEaten = 0;
        this.speed = Utils.calculateSpeed(this.level);
    ...
    eatFood() {
        ...
        // Check for level up
        if (this.foodEaten >= GameConfig.FOOD_PER_LEVEL) {
            this.levelUp();
        }
    ...
    updateUI() {
        ...
        levelElements.forEach(element => {
            if (element) element.textContent = this.level;
        });

        // Update device mode display
        const deviceModeElement = document.getElementById('deviceMode');
        if (deviceModeElement) {
            deviceModeElement.textContent = this.isMobile ? 'Mobile Mode' : 'PC Mode';
        }
    }
