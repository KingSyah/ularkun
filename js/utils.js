...
const GameConfig = {
    ...
    // Mobile speed settings (slower for better UX)
    MOBILE_INITIAL_SPEED: 250, // Much slower start for mobile
    MOBILE_SPEED_INCREASE_PER_LEVEL: 15, // Slower progression
    MOBILE_MIN_SPEED: 120, // Not too fast even at max level
    ...
    // Level progression
    FOOD_PER_LEVEL: 5,
    MOBILE_FOOD_PER_LEVEL: 7, // More food needed per level on mobile for gradual learning
    MAX_LEVEL: 10,
...
const Utils = {
    ...
    // Calculate game speed based on level and device type
    calculateSpeed: (level, isMobile = false) => {
        if (isMobile) {
            // Mobile: slower and more gradual progression
            const speed = GameConfig.MOBILE_INITIAL_SPEED - (level - 1) * GameConfig.MOBILE_SPEED_INCREASE_PER_LEVEL;
            return Math.max(speed, GameConfig.MOBILE_MIN_SPEED);
        } else {
            // Desktop: original speed settings
